...
export const useGoogleStore = create((set) => ({
  ...
  getAllSheetInfo: async (range, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.google.requestGetAlSheetInfo(range);
  ...
  AddRowToSheet: async (range, query, oauthAccessToken, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.google.requestAddRowToSheet(range, query, oauthAccessToken);
      set({ sheets: response.data });
      onSuccess(response.data);
    } catch (error) {
      onFail(error?.response?.data?.msg || 'Có lỗi xảy ra!');
