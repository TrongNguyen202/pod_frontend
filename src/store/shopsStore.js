...
export const useShopsStore = create((set, get) => ({
  ...
  getAllStores: async (onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.stores.requestGetAllStores();
  ...
  createStore: async (data, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.stores.requestCreateStore(data);
  ...
  getStoreById: async (id, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.stores.requestGetStoreById(id);
  ...
  updateStore: async (id, data, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.stores.requestUpdateStore(id, data);
      const index = get().stores.findIndex((item) => item.id === id);
      const newStores = [...requestGet().stores];
  ...
  searchStores: async (query, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.stores.requestSearchStores(query);
  ...
  refreshToken: async (ShopId, onSuccess = () => {}, onFail = () => {}) => {
    try {
      set({ loading: true });
      const response = await RepositoryRemote.stores.requestRefreshToken(ShopId);
      onSuccess(response.data);
    } catch (error) {
      onFail(error?.response?.data?.msg || 'Có lỗi xảy ra!');
    }
